...
export function ChatbotFab() {
  ...
  // Lazy-load chatbot data only when first opened
  useEffect(() => {
    if (!open || loaded) return;
    setLoaded(true);
    supabase
      .from("chatbot_responses")
      .select("id,keyword,question_sample,response,suggested_product_type")
    ...
    <>
      ...
      {open && (
        <div
          ...
          className="fixed bottom-24 right-4 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[380px] h-[520px] max-h-[calc(100vh-8rem)] bg-white rounded-2xl shadow-2xl border border-green-100 flex flex-col overflow-hidden"
        >
